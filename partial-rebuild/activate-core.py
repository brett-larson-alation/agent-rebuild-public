...
##########################################################################################
# Name: Activate Agent System (Manual Login Version)
# Description: Phase 2 of partial rebuild solution. Quickly configures and activates
# a prepared Alation Agent system. Must be run after prepare-core.py
...
#
# Prerequisites:
# 1. prepare-core.py must have been run successfully on this server
# 2. Primary agent must be completely offline before running this script
# 3. You must be a Server Admin user in Alation
# 4. You must use username/password authentication (NOT SSO)
#
# For SSO users: Use activate-sso.py instead of this script.
#
...
# NOTE: BASE_URL, USERNAME, PASSWORD should match prepare-core.py
...
# This MUST be the same agent_id used in install-dormant-core.py
agent_id = 1
...
with requests.Session() as session:
    ...
    try:
        ...
        logger.info("### This script activates a dormant agent prepared by install-dormant-core.py ###")
        ...
        if not subprocess.run(["which", "kratos"], capture_output=True).returncode == 0:
            raise Exception("Kratos binary not found. Did you run install-dormant-core.py first?")
        
        if not subprocess.run(["which", "hydra"], capture_output=True).returncode == 0:
            raise Exception("Hydra binary not found. Did you run install-dormant-core.py first?")
            
        if not subprocess.run(["which", "docker"], capture_output=True).returncode == 0:
            raise Exception("Docker not found. Did you run install-dormant-core.py first?")
        ...
        logger.info("### Generating refresh token for activation... ###")
        # Create a refresh token using Server Admin credentials
        response = session.post(
            f"{BASE_URL}/integration/v1/createRefreshToken/",
            json={"username": USERNAME, "password": PASSWORD, "name": "agent-activate"}
        )
        ...
        logger.info("### Generating access token for activation... ###")
        ...
        logger.error("###   3. Validate authentication credentials ###")
